...
template<class r_option>
void container_base<r_option>::print_all_custom(const char *format,cele3d* faces) {
	int fp=0;
	...
	while(format[fp]!=0) {
		if(format[fp]=='%') {
			...
			if(format[fp]=='n') {
				...
				// class
				voronoicell_neighbor c;
				print_all_custom_internal(c,format,faces);
				return;
			} else if(format[fp]==0) break;
		}
		fp++;
...
void container_base<r_option>::print_all_custom_internal(voronoicell_base<n_option> &c,const char *format,cele3d* faces) {
	fpoint x,y,z;
	int i,j,k,ijk=0,q,fp,h;
int ids=0;
		...
		while(format[fp]!=0) {
			if(format[fp]=='%') {
				...
				switch(format[fp]) {
					...
					case 'i': ids=id[ijk][q]; faces[ids].id=ids;break;

					// Vertex-related output
					case 'w': {faces[ids].nrvertext=c.p; 			
						faces[ids].vertexes = new float*[faces[ids].nrvertext];
						for (h = 0; h < faces[ids].nrvertext; h++)
						{
							faces[ids].vertexes[h] = new float[3];
						}
					}break;
					case 'P': c.output_vertices(faces,ids,x,y,z);break;

					// Face-related output
					case 's': 
						{
							faces[ids].nrfaces=c.number_of_faces();
							faces[ids].facevertextnr = new int[faces[ids].nrfaces];
							faces[ids].kaimynai = new int[faces[ids].nrfaces];
							faces[ids].faceorder = new int*[faces[ids].nrfaces];
						}break;
					case 'a': c.output_face_orders(faces,ids);break;					
					case 't':
						{
							for(h=0;h<faces[ids].nrfaces;h++)
							{
								faces[ids].faceorder[h]=new int[faces[ids].facevertextnr[h]];
							}

							c.output_face_vertices(faces,ids);
						}                    break;
					case 'n': c.output_neighbors(faces,ids);break;
