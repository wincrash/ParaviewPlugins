...
bool voronoicell_base<n_option>::nplane(fpoint x, fpoint y, fpoint z, fpoint rsq, int p_id) {
    int count = 0, i, j, k, lp = up, cp, qp, rp, stack = 0;
    stack2 = 0;
...
template<class n_option>
void voronoicell_base<n_option>::print_edges(vector<vector<int> > &realations) {
    int j;
...
//cout<<"--------------------\n";
    ...
    //    for (j = 0; j < nu[i]; j++) cout << " " << ed[i][j];
     ...
     //   while (j < 2 * nu[i]) cout << " " << ed[i][j++];
        //cout << "    " << ed[i][j];
    	vector<int> temp;
        neighbor.print_edges(temp,i);
        realations.push_back(temp);
...
template<class n_option>
void voronoicell_base<n_option>::output_vertices(cele3d *faces, int ids, fpoint x, fpoint y, fpoint z) {
    if (p == 0) return;
    int k = 0;
    //	os << " " << x+pts[0]*0.5 << " " << y+pts[1]*0.5 << " " << z+pts[2]*0.5 << "";
    faces[ids].vertexes[k][0] = x + pts[0]*0.5;
    faces[ids].vertexes[k][1] = y + pts[1]*0.5;
    faces[ids].vertexes[k][2] = z + pts[2]*0.5;
    k++;
    for (int i = 3; i < 3 * p; i += 3) {
        faces[ids].vertexes[k][0] = x + pts[i]*0.5;
        faces[ids].vertexes[k][1] = y + pts[i + 1]*0.5;
        faces[ids].vertexes[k][2] = z + pts[i + 2]*0.5;
        k++;
        // os << " " << x+pts[i]*0.5 << " " << y+pts[i+1]*0.5 << " " << z+pts[i+2]*0.5 << "";
    }
}
...
template<class n_option>
void voronoicell_base<n_option>::output_face_orders(cele3d *faces, int ids) {
    int i, j, k, l, m, q;
    bool later = false;
    int id = 0;
    for (i = 0; i < p; i++) {
        for (j = 0; j < nu[i]; j++) {
            ...
            if (k >= 0) {
                ...
                if (!later) later = true;
                faces[ids].facevertextnr[id] = q; //cout << q;
                id++;
            }
        }
    }
...
template<class n_option>
void voronoicell_base<n_option>::output_face_vertices(cele3d *faces, int ids) {
    int i, j, k, l, m;
    bool later = false;
    int id = 0;
    int id1 = 0;
    for (i = 0; i < p; i++) {
        for (j = 0; j < nu[i]; j++) {
            ...
            if (k >= 0) {
                if (!later) later = true;
                //cout << "(" << i;
                faces[ids].faceorder[id][id1] = i;
                id1++;
                ed[i][j] = -1 - k;
                l = cycle_up(ed[i][nu[i] + j], k);
                do {
                    //cout << "," << k;
                    faces[ids].faceorder[id][id1] = k;
                    id1++;
                    m = ed[k][l];
                    ed[k][l] = -1 - m;
                    l = cycle_up(ed[k][nu[k] + l], m);
...
template<class n_option>
void voronoicell_base<n_option>::output_neighbors(cele3d *faces, int ids, bool later) {
    neighbor.neighbors(faces, ids, later);
}
 ...
 * \param[in] later A boolean value to determine whether or not to write a
 * space character before the first entry. */
void neighbor_track::neighbors(cele3d *faces, int ids, bool later) {
 ...
 * \param[in] &os An open output stream to write to. */
template<class n_option>
void voronoicell_base<n_option>::output_face_vertices(int **indexes) {
    int i, j, k, l, q = -1, qq = 0, m;
    for (i = 0; i < p; i++) {
        for (j = 0; j < nu[i]; j++) {
            k = ed[i][j];
            if (k >= 0) {
 ...
 * \param[in] &os An open output stream to write to. */
template<class n_option>
void voronoicell_base<n_option>::output_face_orders(vector<int> &faces_kiekis) {
    int i, j, k, l, m, q;
    ;
    for (i = 0; i < p; i++) {
        for (j = 0; j < nu[i]; j++) {
 ...
 * space character before the first entry. */
template<class n_option>
void voronoicell_base<n_option>::output_neighbors(vector<int> &kaimynai){
    neighbor.neighbors(kaimynai);
}
...
inline void neighbor_track::print_edges(vector<int> &temp, int i) {
    //cout << "    (";
    for (int j = 0; j < vc->nu[i]; j++) {
    	temp.push_back(ne[i][j]);
 ...
 * \param[in] later A boolean value to determine whether or not to write a
 * space character before the first entry. */
void neighbor_track::neighbors(vector<int> &kaimynai) {
    int **edp = vc->ed, *nup = vc->nu;
    int i, j, k, l, m;
    for (i = 0; i < vc->p; i++) {
        for (j = 0; j < nup[i]; j++) {
