...
vtkIdType TDEM::findClosestSolutionIndex(double Time)
{
    double dt=1e24;
        ...
        vtkInformationVector *outputVector)
{
    ...
    std::vector<REAL4> HOST_POINTS_INIT;
    {
        ...
        ReadReal4Array(fileInit,HOST_POINTS_INIT,PARTICLE_COUNT*4,"TETRAHEDRA");
        delete fileInit;
    ...
    ReadReal4Array(file,HOST_POINTS,PARTICLE_COUNT*4,"TETRAHEDRA");
    std::vector<int> FIX(PARTICLE_COUNT,0);
    std::vector<int> MATERIAL(PARTICLE_COUNT,0);
    std::vector<int> WALL(PARTICLE_COUNT,0);
    ReadIntArray(file,FIX,PARTICLE_COUNT,"FIX");
    ReadIntArray(file,MATERIAL,PARTICLE_COUNT,"MATERIAL");
    ReadIntArray(file,WALL,PARTICLE_COUNT,"WALL");
    ...
    if(file->exists("PARTICLE_CPU"))
    {
        std::vector<int> HOST_PARTICLE_CPU;
        ReadIntArray(file,HOST_PARTICLE_CPU,PARTICLE_COUNT,"PARTICLE_CPU");
        AddToVTKScalar(cdata,HOST_PARTICLE_CPU,"PARTICLE_CPU");
    }
...
void TDEM::PrintSelf(ostream& os, vtkIndent indent)
{
...
int TDEM::RequestInformation(vtkInformation *vtkNotUsed(request), vtkInformationVector **vtkNotUsed(inputVector), vtkInformationVector *outputVector)
{
    ...
    std::cout<<"TDEM "<<fs::path(FileName).parent_path()<<"   "<<FileName<<"  "<<fileStringStart<<"\n";

    for(auto& p: fs::directory_iterator(fs::path(FileName).parent_path()))
    {
        if (p.path().string().find(".tdem") != std::string::npos)
